    ...
    <Card className="overflow-hidden transition-all-200 hover:shadow-md border border-border/50 hover:border-border/80">
      <Link to={`/project/${project.id}`} className="block h-full">
        <CardHeader className="p-4 pb-2 flex flex-row justify-between items-start">
          <div>
            <CardTitle className="line-clamp-1 mb-1 text-lg">{project.title}</CardTitle>
            <CardDescription className="line-clamp-2">{project.description}</CardDescription>
          </div>
          <DropdownMenu>
            <DropdownMenuTrigger asChild onClick={(e) => e.preventDefault()}>
              <Button variant="ghost" size="icon" className="-mr-2 opacity-70 hover:opacity-100">
        ...
        <CardContent className="p-4 pt-2">
          <div className="flex flex-wrap gap-2 mt-2">
            <Badge className={cn("cursor-default", getStatusColor(project.status))}>
              {project.status.charAt(0).toUpperCase() + project.status.slice(1)}
            </Badge>
            <Badge className={cn("cursor-default", getTypeColor(project.type))}>
