...
export function SortFilterBar({ options, onChange }: SortFilterBarProps) {
  ...
  const hasActiveFilters =
    options.type !== "all" || options.status !== "all" || options.date !== undefined;
    ...
    <div className="bg-white/60 backdrop-blur-md border rounded-xl p-4 mb-6 shadow-sm transition-all-200">
      <div className="flex flex-col space-y-4">
        <div className="flex items-center space-x-2 w-full">
          ...
          <Select
            value={options.sort}
            onValueChange={(value) => updateOptions({ sort: value as SortOption })}
          >
        ...
        {showFilters && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2 animate-fade-in">
            <Select
              value={options.type}
              onValueChange={(value) => updateOptions({ type: value as ProjectType | "all" })}
            >
            ...
            <Select
              ...
              onValueChange={(value) => updateOptions({ status: value as ProjectStatus | "all" })}
            >
              <SelectTrigger className="bg-background/80">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {options.date ? format(options.date, "PPP") : "Filter by date"}
                  </Button>
                </PopoverTrigger>
